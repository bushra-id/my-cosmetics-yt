    ...
    const getRating = () => {
        const randomNumber = (min: number, max: number) => {
          ...
          switch (randomNumber(0 , 5)) {
            case 0:
            return (
                <div className='fles justify-center text-accent pt-4 pb-2'>
                   <AiOutlineStar />
                   <AiOutlineStar />
                   <AiOutlineStar />
                   <AiOutlineStar />
  ...
  return (
    <div className="group cursor-pointer" onClick={() => router.push(`/details/${id}`)} > 
      <div className='relative'>
        ...
        alt={name}
         />
{sale && (<div className='bg-red-600 inline-block absolute top-0 left-0 text-[14px] text-white
rounded-md px-2 py-[2px] m-4'>SALE!
</div>)} 

 <div className='absolute top-0 left-0 w-full h-full bg-[#00000050] opacity-0 transition-capacity
